...
START_BATCH_LOG_FILE=$LOG_DIR/START_BATCH_Audit_`date +%Y-%m-%d-%H_%M_%S`.log

echo " in update step audit "

touch $START_BATCH_LOG_FILE

echo " Batch start Process ...." > $START_BATCH_LOG_FILE

echo "SCRIPT NAME: $script_name" >> $START_BATCH_LOG_FILE
...
plsql_code_status=` sqlplus -s ${CONNECT_STRING} <<EOF | grep -v 'Connected.' | grep -v '^$'
     ...
     begin
            pkg_fwk_utl.p_start_batch_id (STEP_ID, o_batch_id, o_err_code);
