...
FEED_NAME_PATTERN=$1
OUTPUT_FILE_NAME=$FEED_NAME_PATTERN"_lst.txt"
#Calling configuration file
. /u02/Scripts/ETL_Indirect_File_list_CFG.cfg
...
count=`ls -lrt $SRC_FILE_PATH | grep -i '.csv' | grep -i "$FEED_NAME_PATTERN" | wc -l` 
...
if [[ $count > 0 ]] 
then 
ls -lrt $SRC_FILE_PATH | grep -i '.csv' |awk -F ' ' '{print $9}' | grep -i "$FEED_NAME_PATTERN" >  $SRC_FILE_PATH/$OUTPUT_FILE_NAME
echo " Below files added to Indirect list for processing.." >>$LOG_FILE
ls -lrt $SRC_FILE_PATH | grep -i '.csv'|awk -F ' ' '{print $9}' | grep -i "$FEED_NAME_PATTERN" >> $LOG_FILE
